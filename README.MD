...
* making a class singleton can make it difficult to test its clients because you can't create mocks

#### 1 final field member
```java
...
```
* nothing the client does can change the fact that one instance will exist once the Elvis class is initialized (unless reflection)
* defend against reflection attack by modifying the constructor to throw an exception when asked to create a second instance
...
```
* the advantage is you are flexibility to change what to return (you could make it not singleton later)
* works with generic method if you need it
* method reference can be used as a supplier, for example `Elvis::instance` is a `Supplier<Elvis>`
...
* Otherwise, each time a serialized instance is deserialized, a new instance will be created (see page 18 example)
...
* can’t use this approach if your singleton must extend a superclass other than Enum(though you can declare an enum to implement interfaces)

### Item 4 Enforce noninstantiability with a private constructor
* occassionally we have helper classes that is just a grouping of static methods and static fields
* for example `java.lang.Math`, `java.util.Arrays`, and `java.util.Collections`
* attempting to enforce noninstantiability by making a class abstract does not work (because it can be subclassed)
...
* if you have a class that depends on some resource to function, pass the resouce into the constructor when creating a new instance
...
* simliarly, you can also pass in a resource factory instead
---
* static utility classes and singletons are inappropriate for classes whose behavior is parameterized by an underlying resource
* the example given this chapter is `SpellChecker` requires `Dictionary` to function
```java
...
```
* the problem with these is it cannot support changing the dictionary object
* there are many genuine need to swap different dictionary (resources), including testing
* attempt to support multiple dictionaries by making the dictionary field nonfinal and adding a method to change the dictionary in an existing spell checker is awkward, error-prone,and unworkable in a concurrent setting

### Item 6 Avoid creating unnecessary objects
* an object can always be reused if it is immutable
* never create a string from constructor with `new String("something"")` as "something" is already a string itself
* since string is immutable, java internally manage a [string pool](https://www.baeldung.com/java-string-pool) to reuse string
* if you call the constructor with `new` then a new object is always created, but if you call a static factory methods you might reusing the same immutable object
    * for example `Boolean.valueOf(String)` instead of `new Boolean(String)` which is deprecated
    
#### not obvious unnecessary objects creation
* regular expression String.matches internally creates a Pattern instance for the regular expression and uses it only once, after which it is eligible for garbage collection
...
* it is not suitable for repeated use in performance-ciritcal situations
```java
...
```
* fix: explicitly compile the regular expression into a `Pattern` and cache it
```java
...
```
* note, lazy initializing the field the first time the method is invoked is not recommended
...
* every call to keySet on a given Map object may return the same Set instance because they are all refering to the same collection of items
---
* autoboxing allows programmers to mix primitive and boxed primitive types, boxing and unboxing automatically as needed
```java
...
```
* declaring the `sum` `Long` instead of `long` means every i is autoboxed into a `Long` object before adding to sum, creating extra object on every loop
#### watch out
* the creation and reclamation of small objects whose constructors do little explicit work is cheap, especially on modern JVM implementations    
* avoiding object creation by maintaining your own object pool on lightweight objects only clutters your code, increases memory footprint, and harms performance
    * ok for expensive object like database connection
...
```
* when this stack pop something, the object is still referenced by the `elements` array
...
* fix the above memory leak by null out the unused reference
```java
public Object pop() {
    if (size == 0)
        throw new EmptyStackException();
...
* the best way to eliminate an obsolete reference is to let the variable that contained the reference fall out of scope which occurs naturally if you define each variable in the narrowest possible scope
* if a class manages its own memory than you should null out a reference
---
* another common source of memory leaks is caches if you forget and leave it in the cache long after it becomes irrelevant
* you may try a `WeakHashMap` that its entries will be removed automatically after the key outside of the cache become obsolete
...
---
* memory leaks are typically discovered only as a result of careful code inspection or with the aid of a debugging tool known as a heap profiler

### Item 8 Avoid finalizers and cleaners
* the `finalizer()` method is a method provided by the root Object class, it is called before the garbage collection for a particular object

#### Problems with finalizer and cleaners
* there is no guarantee they’ll be executed promptly
    * you should never do anything time-critical in a finalizer or cleaner like close files because open file descriptors are a limited resource
* the promptness with which finalizers and cleaners are executed is varies widely across implementation
* there is no guarantee that they’ll run at all
    * you should never depend on a finalizer or cleaner to update persistent state, such as releasing a persistent lock
* uncaught exception thrown during finalization is ignored, and finalization of that object terminates
     * normally, an uncaught exception will terminate the thread and print a stack trace, but not if it occurs in a finalizer
* there is a severe performance penalty for using finalizers and cleaners     
* they also open your class up to finalizer attacks
    *  if an exception is thrown from a constructor or its serialization equivalents, the finalizer of a malicious subclass can run on the partially constructed object that should not have existed
---
* instead, have your class implement AutoCloseable, and require its clients to invoke the close method on each instance when it is no longer needed
*  the instance must keep track of whether it has been closed: the close method must record in a field that the object is no longer valid
#### valid use case of finalizer and cleaners
* act as a safety net in case the owner of a resource neglects to call its close method. While there’s no guarantee that the cleaner or finalizer will run promptly (or at all), it is better to free the resource late than never if the client fails to do so
* native peer is not a normal object, the garbage collector doesn’t know about it and can’t reclaim it when its Java peer is reclaimed. A cleaner or finalizer may be an appropriate vehicle for this task

### Item 9 Prefer try-with-resources to try-finally
```java
* some java libraries require you to manually close the resource, like InputStream,OutputStream, and java.sql.Connection
...
```
* it get worse when you have multiple resource
```java
...
```
* another problem is `close()` in the finally block could still fail for the same reason try fail, for example a failure in the underlying physical device
* there is no record of the first exception in the exception stack trace, which can greatly complicate debugging- usually it’s the first exception that you want to see
#### Solution
* try-with-resources statement will automatically handle closing the resouce in case of exception
...
```
* if exceptions are thrown by both the try and the (invisible) close, the latter exception is suppressed in favor of the former
...
* you can also access them programmatically with the getSuppressed method in `Throwable` 
* of course you can still have a catch clause like a normal try-catch
...
```
* this only work because map class has a method call merge that accept [parameters key and value, and a BiFunction](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/util/Map.html#merge(K,V,java.util.function.BiFunction))
